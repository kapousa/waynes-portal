    ...
    useEffect(() => {
        const fetchCategories = async () => {
            ...
            try {
                ...
                // Ensure we handle both Strapi v4/v5 response formats
                ...
                >
                    <div className={cn("flex items-center gap-3 mb-2", isRTL && "flex-row-reverse")}>
                        ...
                        <h1 className="text-2xl md:text-3xl font-semibold text-foreground">
                            {/* CTO FIX: Use optional chaining to prevent crashes if user is null */}
                    ...
                    {!isLoading && !error && categories.length > 0 && (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
                            ...
                            {categories.map((category: any) => {
                                ...
                                // Use the helper function to get the actual component
                                const IconComponent = getIconForCategory(category.slug || '');

                                return (
                                    <CategoryCard
                                        key={category.id}
                                        title={language === 'ar' && category.nameAr ? category.nameAr : category.name}
                                        description={category.description || ''}
                                        slug={category.slug || ''}
                                        // Pass 'articleCount' to the 'documentCount' prop
                                        documentCount={articleCount}
                                        icon={IconComponent} // Pass the component, NOT a string
                                    />
                                );
                            })}
                        </div>
