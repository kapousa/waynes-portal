// Strapi API Configuration
export const STRAPI_URL = import.meta.env.VITE_STRAPI_URL || 'http://localhost:1337';

// API Token for Strapi requests (for production, move to secure environment variables)
const STRAPI_API_TOKEN = '';

interface LoginCredentials {
    identifier: string;
    password: string;
}

interface AuthResponse {
    jwt: string;
    user: {
        id: number;
        username: string;
        email: string;
    };
}

export interface Category {
    id: number;
    documentId: string; // Added from your Postman response
    name: string;
    slug: string | null;
    description: string | null;
    nameAr?: string;        // Keep these for your bilingual support
    descriptionAr?: string;
    icon?: string;          // Keep this for your UI icons
    createdAt: string;
    updatedAt: string;
    publishedAt: string;
    // This allows the code to work even if Strapi 4 'attributes' are present
    attributes?: any;
}

// --- Inside api.ts ---
export interface Document {
    id: number;
    documentId: string; // Strapi 5 uses this
    title: string;
    titleAr?: string;
    description: string;
    descriptionAr?: string;
    tags: string[];
    file: {
        url: string;
        name: string;
        size: number;
    };
    createdAt: string;
    updatedAt: string;
    // Keep attributes as optional for backward compatibility if needed
    attributes?: any;
}

// --- Token Management Helpers ---

export const getToken = (): string | null => {
    return localStorage.getItem('waynes_token');
};

export const setToken = (token: string): void => {
    localStorage.setItem('waynes_token', token);
};

export const removeToken = (): void => {
    localStorage.removeItem('waynes_token');
    localStorage.removeItem('waynes_user');
};

export const getCurrentUser = () => {
    const user = localStorage.getItem('waynes_user');
    return user ? JSON.parse(user) : null;
};

export const setCurrentUser = (user: AuthResponse['user']): void => {
    localStorage.setItem('waynes_user', JSON.stringify(user));
};

export const isAuthenticated = (): boolean => {
    return !!getToken();
};

// --- API Request Helper ---

const apiRequest = async <T>(
    endpoint: string,
    options: RequestInit = {},
    locale?: string
): Promise<T> => {
    const url = new URL(`${STRAPI_URL}/api${endpoint}`);

    if (locale) {
        url.searchParams.set('locale', locale);
    }

    const headers: Record<string, string> = {
        'Content-Type': 'application/json',
    };

    /**
     * CTO FIX: Header Logic for Auth
     * 1. If we are logging in, do NOT send any Authorization header.
     * 2. For all other requests, use the logged-in User's JWT first.
     * 3. Fallback to the Master API Token if no user JWT exists.
     */
    if (!endpoint.includes('/auth/local')) {
        const userToken = getToken();
        const tokenToUse = userToken || STRAPI_API_TOKEN;
        headers['Authorization'] = `Bearer ${tokenToUse}`;
    }

    const response = await fetch(url.toString(), {
        ...options,
        headers: {
            ...headers,
            ...options.headers,
        },
    });

    if (!response.ok) {
        const error = await response.json().catch(() => ({message: 'Request failed'}));
        // Provides better error messages for validation failures
        throw new Error(error.error?.message || error.message || 'Request failed');
    }

    return response.json();
};

// --- Auth Endpoints ---

export const login = async (credentials: LoginCredentials): Promise<AuthResponse> => {
    // apiRequest will now skip headers for this specific call
    const response = await apiRequest<AuthResponse>('/auth/local', {
        method: 'POST',
        body: JSON.stringify(credentials),
    });

    setToken(response.jwt);
    setCurrentUser(response.user);

    return response;
};

export const logout = (): void => {
    removeToken();
};

// --- Data Endpoints ---

export const getCategories = async (locale: string = 'en'): Promise<{ data: Category[] }> => {
    return apiRequest<{ data: Category[] }>('/categories?populate=*', {}, locale);
};

export const getDocuments = async (
    locale: string = 'en',
    categorySlug?: string
): Promise<{ data: Document[] }> => {
    let endpoint = '/articles?populate=*';
    if (categorySlug) {
        endpoint += `&filters[category][slug][$eq]=${categorySlug}`;
    }
    return apiRequest<{ data: Document[] }>(endpoint, {}, locale);
};

export const searchDocuments = async (
    query: string,
    locale: string = 'en'
): Promise<{ data: Document[] }> => {
    const endpoint = `/articles?populate=*&filters[$or][0][title][$containsi]=${query}&filters[$or][1][description][$containsi]=${query}`;
    return apiRequest<{ data: Document[] }>(endpoint, {}, locale);
};

export const getDocument = async (
    id: number,
    locale: string = 'en'
): Promise<{ data: Document }> => {
    return apiRequest<{ data: Document }>(`/articles/${id}?populate=*`, {}, locale);
};

export type {Category, Document, AuthResponse};